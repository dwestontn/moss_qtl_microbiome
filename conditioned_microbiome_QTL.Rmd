...
lmSlopePvalue <- tidy(lm_results, fitgrowth) %>% select(genotypeID, chamb_micro, term, estimate, p.value) %>% filter(term =="sampletime")

lmRsquare <- glance(lm_results, fitgrowth) %>% select(genotypeID, chamb_micro, r.squared)
...
```{r}
datER<- datER %>% mutate(cham_micro = paste(chamber, microbiome, sep = "_"))

datERdiff<-datER %>%
    group_by(genotypeID, cham_micro) %>%
    mutate(growth.diff = area_cm2 - lag(area_cm2, default = first(area_cm2)))

  ggplot(datERdiff, aes(color = cham_micro, y= growth.diff, x= timepoint)) +
  geom_boxplot(position=position_dodge(.8)) +
  geom_jitter(position=position_dodge(.8)) +
  theme(axis.text=element_text(size=12),
        axis.text.x = element_text(angle = 90, vjust = 0.5),
...
test<-datERdiff %>% select(timepoint, genotypeID, area_cm2, cham_micro) %>% filter(timepoint == "initial" | timepoint =="week1")

datERdiff.week1<-test %>%
    group_by(genotypeID, cham_micro) %>%
    mutate(week1.diff = area_cm2 - lag(area_cm2, default = first(area_cm2))) %>% filter(timepoint =="week1")

  ggplot(datERdiff.week1, aes(color = cham_micro, y= week1.diff, x= cham_micro)) +
  geom_boxplot(position=position_dodge(.8)) +
  geom_jitter(position=position_dodge(.8)) +
  theme(axis.text=element_text(size=12),
        axis.text.x = element_text(angle = 90, vjust = 0.5),
